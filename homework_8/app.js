...
form.addEventListener('submit', function (e) {
    ...
    if (!title.value || !text.value) {
        alertMessage('No title or text', 'alert-danger');
        return;
    }
    ...
    if (dataTaskId) {
        ...
        alertMessage('Todo was edited', 'alert-info');
    } else {
        addNewTodoToStorage(title.value, text.value);
        alertMessage('New todo was added', 'alert-info');
    }
...
function deleteTodoFromView(id) {
    ...
    alertMessage('Todo was deleted', 'alert-info');
}
...
function alertMessage(message, className = 'alert-info') {
    removeAlert();
    formCol.insertAdjacentHTML('afterbegin', alertTemplate(message, className));
